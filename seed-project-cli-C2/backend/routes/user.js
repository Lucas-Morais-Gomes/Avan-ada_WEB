...
router.post("/signup", async (req, res) => {
    try {
        ...
        if (!req.body.email || !req.body.password || !req.body.country || !req.body.gender) {
            console.error("Campos obrigatórios ausentes: Email, senha, país ou gênero"); // Mensagem de erro
            return res.status(400).json({ error: "Email, senha, país e gênero são obrigatórios" }); // Responde com um erro
        }
...
router.get("/", async (req, res, next) => {
    try {
      ...
      res.render("/", {
        usuarios: usersEncontrados, // Renderiza a página com os usuários encontrados
      });
    } catch (err) {
