...
router.delete("/:_id", async function (req, res, next) {
  ...
  try {
    ...
    res.status(200).json({ myMsgSucesso: "Mensagem excluída com sucesso no banco de dados" }); // Mensagem de sucesso
  } catch (err) {
    console.error('Erro ao excluir a mensagem no banco de dados:', err);
    return res.status(500).json({
      myErrorTitle: "Server-Side: Um erro aconteceu ao excluir a mensagem no banco de dados", // Título do erro
      myError: err, // Mensagem de erro
    });
  }
});
...
router.put("/:_id", async function (req, res, next) {
  ...
  try {
    ...
    res.status(200).json({ myMsgSucesso: "Mensagem atualizada com sucesso", updatedMessage }); // Mensagem de sucesso
  } catch (err) {
    console.error('Erro ao atualizar a mensagem no banco de dados:', err);
    return res.status(500).json({
      myErrorTitle: "Server-Side: Um erro aconteceu ao atualizar a mensagem no banco de dados", // Título do erro
      myError: err, // Mensagem de erro
    });
  }
});
